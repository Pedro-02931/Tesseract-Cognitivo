# Windows 10 - Automação via PowerShell

Bom, a atividade teoricamente era para ser simples, então dado que não gosto de fazer nada pequeno, decidi documentar como pode-se automatizar a criação de usuários, criação de pastas, atribuições percursivas, compartilhamento e teste SMB.

## New-CustomUsers

Bom, aqui eu precisei criar uma variavel que carrega uma lista de objetos através do hash table (@).

No caso, crie a variavel users que carrega um grupo de chaves e valores
